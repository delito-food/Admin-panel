...
export default function DeliveryPartnersPage() {
    ...
    // Suspend modal
    const [suspendTarget, setSuspendTarget] = useState<DeliveryPartner | null>(null);
    ...
    const handleSuspend = async () => {
        ...
        try {
            const res = await fetch('/api/delivery/suspend', {
                ...
                body: JSON.stringify({ deliveryPersonId: suspendTarget.deliveryPersonId, reason: suspensionReason, notes: suspensionNotes }),
            });
            ...
            if (result.success) {
                ...
                setSuspendError(result.error || 'Failed to suspend partner');
            }
        } catch {
    ...
    const handleReinstate = async (partnerId: string) => {
        if (!confirm('Reinstate this delivery partner?')) return;
        setProcessing(partnerId);
        try {
            const res = await fetch(`/api/delivery/suspend?deliveryPersonId=${partnerId}&adminId=admin`, { method: 'DELETE' });
            const result = await res.json();
                                        ...
                                        <td>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                                <div style={{
                                                    ...
                                                    <span style={{
                                                        display: partner.profilePhotoUrl ? 'none' : 'flex',
                                        ...
                                        <td>
                                            ...
                                            {(partner.codPending || 0) > 0 && (
                                                <p style={{ fontSize: '0.65rem', color: '#EF4444', marginTop: 2 }}>
                                                    COD: {formatCurrency(partner.codPending)}
                                                </p>
                                            )}
                                        </td>
                                                                    ...
                                                                    <button
                                                                        onClick={() => {
                                                                            ...
                                                                            if (partner.status === 'active') {
                                                                                setSuspendTarget(partner);
                                                                            } else {
                                                                                handleReinstate(partner.deliveryPersonId);
                                                                            }
                                                                        }}
                                                                        className="dropdown-item"
            ...
            {suspendTarget && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
                    <div style={{ background: 'var(--surface)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 440, margin: '0 16px', border: '1px solid var(--glass-border)' }}>
                        <h2 style={{ fontWeight: 700, fontSize: '1.1rem', color: 'var(--foreground)', marginBottom: 4 }}>🚫 Suspend Partner</h2>
                        <p style={{ fontSize: '0.85rem', color: 'var(--foreground-secondary)', marginBottom: 18 }}>
                            Suspending <strong style={{ color: 'var(--foreground)' }}>{suspendTarget.fullName}</strong> will prevent them from accepting new deliveries.
                        </p>
                        <div style={{ marginBottom: 14 }}>
                            ...
                            >
                                ...
                                {['Policy Violation', 'Multiple Customer Complaints', 'Improper Behavior', 'Documents Expired', 'Fraudulent Activity', 'Non-compliance with Platform Rules', 'Extended Inactivity', 'Other (See Notes)'].map(r => (
