...
const PIE_COLORS = ['#F4511E', '#FF9904', '#10B981', '#8B5CF6', '#EC4899'];
...
function StatCard({ title, value, icon: Icon, color, sub }: { title: string; value: string | number; icon: React.ElementType; color: string; sub?: string }) {
    const colors: Record<string, string> = { primary: '#F4511E', success: '#10B981', warning: '#F59E0B', danger: '#EF4444', accent: '#8B5CF6' };
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card" style={{ padding: 20 }}>
            <h3 style={{ fontSize: '0.9rem', fontWeight: 600, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8, color: 'var(--foreground)' }}>
                <Icon size={16} color={iconColor || '#F4511E'} /> {title}
            </h3>
            {children}
        </motion.div>
    );
...
export default function AdvancedReportsPage() {
    ...
    const formatCurrency = (amount: number) => new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 1 }).format(amount);
                    ...
                    <div style={{ height: 320 }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={data.revenueByVendor.slice(0, 10)} layout="vertical">
                                ...
                                <Bar dataKey="totalRevenue" fill="#F4511E" radius={[0, 4, 4, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </ChartCard>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: 16 }}>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card" style={{ overflow: 'hidden' }}>
                    <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)' }}>
                        <h3 style={{ fontSize: '0.9rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8, margin: 0 }}><Store size={16} color="#F4511E" /> Vendor Performance</h3>
                    </div>
                                ...
                                {data.revenueByVendor.slice(0, 10).map((v, i) => (
                                    <tr key={v.vendorId}>
                                        <td><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                            <span style={{ width: 22, height: 22, borderRadius: '50%', background: 'rgba(244,81,30,0.1)', color: '#F4511E', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65rem', fontWeight: 700, flexShrink: 0 }}>{i + 1}</span>
                ...
                <div style={{ height: 280 }}>
                    <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={data.peakHours}>
                            <defs><linearGradient id="hourGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#F4511E" stopOpacity={0.3} /><stop offset="100%" stopColor="#F4511E" stopOpacity={0} /></linearGradient></defs>
                            <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                            <XAxis dataKey="hourLabel" tick={{ fontSize: 10 }} interval={1} />
                            <YAxis />
                            <Tooltip contentStyle={tt} />
                            <Area type="monotone" dataKey="orderCount" stroke="#F4511E" strokeWidth={2} fill="url(#hourGrad)" name="Orders" />
                        </AreaChart>
                    </ResponsiveContainer>
                </div>
            </ChartCard>
                ...
                <div style={{ height: 280 }}>
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data.ordersByDayOfWeek}>
                            ...
                            <Legend />
                            <Bar yAxisId="left" dataKey="orderCount" name="Orders" fill="#F4511E" radius={[4, 4, 0, 0]} />
...
function RetentionTab({ data, fmt }: { data: ReportsData; fmt: (n: number) => string }) {
    ...
    const dist = [
        ...
        { name: '10+ Orders', value: r.customersByOrderCount.moreThanTen, color: '#F4511E' },
    ].filter(i => i.value > 0);
    ...
    const segments = [
        ...
        { label: 'VIP (10+)', count: r.customersByOrderCount.moreThanTen, c: '#F4511E', bg: 'rgba(244,81,30,0.06)', bc: 'rgba(244,81,30,0.15)' },
    ];
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card" style={{ overflow: 'hidden' }}>
                <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)' }}>
                    <h3 style={{ fontSize: '0.9rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8, margin: 0 }}><Users size={16} color="#F4511E" /> Top Customers</h3>
                </div>
                <div style={{ overflowX: 'auto' }}>
                    <table className="table-premium"><thead><tr>
                        <th>#</th><th>Customer</th><th style={{ textAlign: 'right' }}>Orders</th><th style={{ textAlign: 'right' }}>Spent</th><th style={{ textAlign: 'right' }}>Last Order</th>
                            ...
                            {r.topCustomers.map((c, i) => (
                                <tr key={c.customerId}>
                                    <td><span style={{ width: 22, height: 22, borderRadius: '50%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65rem', fontWeight: 700, background: i < 3 ? '#F4511E' : 'var(--foreground-secondary)', color: 'white' }}>{i + 1}</span></td>
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: 16 }}>
                    ...
                    <div style={{ height: 280 }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={d.deliveryTimeDistribution}>
                                ...
                                <Bar dataKey="count" name="Deliveries" fill="#F4511E" radius={[4, 4, 0, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </ChartCard>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card" style={{ padding: 20 }}>
                    <h3 style={{ fontSize: '0.9rem', fontWeight: 600, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8 }}><Store size={16} color="#F4511E" /> Delivery Time by Vendor (Fastest)</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        <div style={{ flex: 1, height: 6, borderRadius: 100, background: 'var(--border)' }}>
                                            <div style={{ width: `${Math.min((v.averageTime / 60) * 100, 100)}%`, height: '100%', borderRadius: 100, background: '#F4511E' }} />
                                        </div>
