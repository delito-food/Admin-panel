    ...
    try {
        ...
        ordersSnapshot.docs.forEach(doc => {
            ...
            if (status === 'delivered' || status === 'completed') {
                ...
                // Delivery partner earns: stored deliveryPersonEarnings or Base₹10 + ₹6.5/km (₹15 min)
                const distanceKm = order.distanceKm || 0;
                const partnerEarnings = order.deliveryPersonEarnings ||
                    (distanceKm > 0 ? Math.max(15, Math.round((10 + distanceKm * 6.5) * 10) / 10) : 15);
                deliveryPerformance[deliveryPersonId].totalEarnings += partnerEarnings;
