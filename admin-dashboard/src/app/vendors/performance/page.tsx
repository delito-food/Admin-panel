...
function StatCard({ title, value, icon: Icon, color, sub }: { title: string; value: string | number; icon: React.ElementType; color: string; sub?: string }) {
    const colors: Record<string, string> = { primary: '#F4511E', success: '#10B981', warning: '#F59E0B', danger: '#EF4444' };
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: 16 }}>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card" style={{ padding: 20 }}>
                    <h3 style={{ fontSize: '0.95rem', fontWeight: 600, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                        <TrendingUp size={18} color="#F4511E" /> Top 10 Vendors by Revenue
                    </h3>
                    <div style={{ height: 300 }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={topVendorsChart} layout="vertical">
                                ...
                                <Bar dataKey="revenue" fill="#F4511E" radius={[0, 4, 4, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </motion.div>
                                ...
                                : filteredVendors.map(v => (
                                    <tr key={v.vendorId} onClick={() => setSelectedVendor(v)} style={{ cursor: 'pointer' }}>
                                        <td><div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                            <div style={{ width: 36, height: 36, borderRadius: 8, background: 'rgba(244,81,30,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                                                {v.shopImageUrl ? <img src={v.shopImageUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Store size={16} color="#F4511E" />}
                                            </div>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setSelectedVendor(null)}>
                    <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} onClick={e => e.stopPropagation()} className="glass-card" style={{ padding: 24, maxWidth: 600, width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 20 }}>
                            <div style={{ width: 56, height: 56, borderRadius: 14, background: 'rgba(244,81,30,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                                {selectedVendor.shopImageUrl ? <img src={selectedVendor.shopImageUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Store size={24} color="#F4511E" />}
                            </div>
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                                    {selectedVendor.cuisineTypes.map(c => (
                                        <span key={c} style={{ padding: '2px 10px', borderRadius: 100, background: 'rgba(244,81,30,0.1)', color: '#F4511E', fontSize: '0.65rem', fontWeight: 600 }}>{c}</span>
                                    ))}
                                </div>
                            </div>
