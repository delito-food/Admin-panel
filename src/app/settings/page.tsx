...
export default function SettingsPage() {
    ...
    const tabColors: Record<TabId, string> = { platform: '#F4511E', notifications: '#FF9904', security: '#10B981', database: '#EF4444' };
                    ...
                    <motion.div key={activeTab} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.2 }} className="glass-card" style={{ padding: 24 }}>
                        ...
                        {activeTab === 'platform' && (
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 14 }}>
                                {[
                                    { key: 'platformCommission', label: 'Platform Commission (%)', desc: 'Commission charged on each order', icon: DollarSign, color: '#F4511E', val: platformSettings.platformCommission },
                                    ...
                                    return (
                                        <div key={item.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 16, borderRadius: 14, background: 'var(--surface)', border: '1px solid var(--border)', transition: 'all 0.2s' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                                <div style={{ width: 36, height: 36, borderRadius: 10, background: checked ? 'rgba(244,81,30,0.1)' : 'var(--surface-hover)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.25s' }}>
                                                    <item.icon size={17} color={checked ? '#F4511E' : 'var(--foreground-secondary)'} />
                                                </div>
                                                <div>
