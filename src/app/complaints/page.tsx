...
const refundStatusConfig: Record<string, { label: string; bg: string; color: string }> = {
    FULL_REFUNDED: { label: 'Full Refund', bg: 'rgba(16,185,129,0.12)', color: '#059669' },
    HALF_REFUNDED: { label: 'Partial Refund', bg: 'rgba(245,158,11,0.12)', color: '#D97706' },
    PARTIAL: { label: 'Partial Refund', bg: 'rgba(245,158,11,0.12)', color: '#D97706' },
    COMPLETED: { label: 'Refunded', bg: 'rgba(16,185,129,0.12)', color: '#059669' },
    PROCESSED: { label: 'Refunded', bg: 'rgba(16,185,129,0.12)', color: '#059669' },
    REFUNDED: { label: 'Refunded', bg: 'rgba(16,185,129,0.12)', color: '#059669' },
    PENDING: { label: 'Pending', bg: 'rgba(245,158,11,0.12)', color: '#D97706' },
    IN_PROGRESS: { label: 'Processing', bg: 'rgba(59,130,246,0.12)', color: '#2563EB' },
    PROCESSING: { label: 'Processing', bg: 'rgba(59,130,246,0.12)', color: '#2563EB' },
    REJECTED: { label: 'Denied', bg: 'rgba(239,68,68,0.12)', color: '#DC2626' },
    DENIED: { label: 'Denied', bg: 'rgba(239,68,68,0.12)', color: '#DC2626' },
    FAILED: { label: 'Failed', bg: 'rgba(239,68,68,0.12)', color: '#DC2626' },
};

function getRefundDisplay(refundStatus: string, refundAmount: number, orderTotal: number) {
    if (!refundStatus || refundStatus === '') {
        return { label: 'Requested', bg: 'rgba(168,85,247,0.12)', color: '#9333EA' };
    }
    const upper = refundStatus.toUpperCase();
    // Check known statuses first
    if (refundStatusConfig[upper]) return refundStatusConfig[upper];
    // Fallback: infer from amount
    if (upper.includes('COMPLET') || upper.includes('SUCCESS')) {
        const isFullRefund = refundAmount > 0 && orderTotal > 0 && refundAmount >= orderTotal * 0.95;
        return isFullRefund
            ? { label: 'Full Refund', bg: 'rgba(16,185,129,0.12)', color: '#059669' }
            : { label: 'Partial Refund', bg: 'rgba(245,158,11,0.12)', color: '#D97706' };
    }
    return { label: refundStatus.replace(/_/g, ' '), bg: 'rgba(107,114,128,0.1)', color: '#6B7280' };
}
                                        ...
                                        <tr key={complaint.complaintId} style={{ cursor: 'pointer' }} onClick={() => openComplaintDetails(complaint)}>
                                            ...
                                            <td style={{ textAlign: 'center' }}>
                                                {complaint.refundRequested ? (
                                                    (() => {
                                                        const rd = getRefundDisplay(complaint.refundStatus, complaint.refundAmount, complaint.orderTotal);
                                                        return (
                                                            <span style={{
                                                                display: 'inline-block', padding: '3px 8px', borderRadius: 100, fontSize: '0.65rem', fontWeight: 600,
                                                                background: rd.bg, color: rd.color,
                                                            }}>
                                                                {rd.label}
                                                            </span>
                                                        );
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
                            ...
                            {selectedComplaint.refundRequested && (
                                (() => {
                                    const rd = getRefundDisplay(selectedComplaint.refundStatus, selectedComplaint.refundAmount, selectedComplaint.orderTotal);
                                    const isProcessed = ['FULL_REFUNDED', 'HALF_REFUNDED', 'COMPLETED', 'PROCESSED', 'REFUNDED'].includes(selectedComplaint.refundStatus?.toUpperCase?.() || '');
                                    return (
                                        <div style={{ padding: 16, borderRadius: 12, background: 'rgba(168,85,247,0.04)', border: `1px solid ${rd.color}33`, marginBottom: 20 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
                                                <div style={{ flex: 1 }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                                                        <span style={{ fontWeight: 700, fontSize: '0.9rem', color: rd.color }}>
                                                            💰 Refund {isProcessed ? 'Processed' : 'Requested'}
                                                        </span>
                                                        <span style={{
                                                            display: 'inline-block', padding: '2px 8px', borderRadius: 100,
                                                            fontSize: '0.65rem', fontWeight: 600, background: rd.bg, color: rd.color,
                                                        }}>
                                                            {rd.label}
                                                        </span>
                                                    </div>
                                                    <div style={{ display: 'flex', gap: 16, fontSize: '0.8rem', color: 'var(--foreground-secondary)' }}>
                                                        <span>Amount: <strong style={{ color: 'var(--foreground)' }}>{formatCurrency(selectedComplaint.refundAmount || selectedComplaint.orderTotal)}</strong></span>
                                                        <span>Order Total: <strong style={{ color: 'var(--foreground)' }}>{formatCurrency(selectedComplaint.orderTotal)}</strong></span>
                                                    </div>
                                                    {selectedComplaint.refundAmount > 0 && selectedComplaint.orderTotal > 0 && (
                                                        <div style={{ marginTop: 6 }}>
                                                            <div style={{ width: '100%', height: 6, borderRadius: 3, background: 'var(--border)', overflow: 'hidden' }}>
                                                                <div style={{
                                                                    width: `${Math.min(100, (selectedComplaint.refundAmount / selectedComplaint.orderTotal) * 100)}%`,
                                                                    height: '100%', borderRadius: 3,
                                                                    background: rd.color,
                                                                    transition: 'width 0.3s ease',
                                                                }} />
                                                            </div>
                                                            <span style={{ fontSize: '0.7rem', color: 'var(--foreground-secondary)', marginTop: 2, display: 'inline-block' }}>
                                                                {Math.round((selectedComplaint.refundAmount / selectedComplaint.orderTotal) * 100)}% of order total
                                                            </span>
                                                        </div>
                                                    )}
                                                </div>
                                                {!isProcessed && selectedComplaint.orderId && (
                            ...
                            <div style={{ display: 'flex', gap: 10, marginTop: 20, alignItems: 'center' }}>
                                {selectedComplaint.orderId && !selectedComplaint.refundRequested && !['FULL_REFUNDED', 'HALF_REFUNDED', 'COMPLETED', 'PROCESSED', 'REFUNDED'].includes(selectedComplaint.refundStatus?.toUpperCase?.() || '') && (
                                    <button onClick={openRefundModal} style={{
                                        display: 'inline-flex', alignItems: 'center', gap: 4,
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                                <div>
                                    ...
                                    <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
                                        ...
                                        <button onClick={() => setRefundAmount(Math.round(selectedComplaint.orderTotal * 0.75))} style={{ padding: '4px 10px', borderRadius: 6, fontSize: '0.7rem', fontWeight: 500, background: 'rgba(59,130,246,0.1)', color: '#3B82F6', border: 'none', cursor: 'pointer' }}>75%</button>
                                        ...
                                        <button onClick={() => setRefundAmount(Math.round(selectedComplaint.orderTotal * 0.25))} style={{ padding: '4px 10px', borderRadius: 6, fontSize: '0.7rem', fontWeight: 500, background: 'rgba(239,68,68,0.1)', color: '#EF4444', border: 'none', cursor: 'pointer' }}>25%</button>
                                    </div>
                                </div>
                                <div>
